...
class DataModel {
  ...
  var containsUserCreatedPark: Bool {
    for park in allParks {
        if park.userCreated == true {
            return true
         }
     }
     return false
  ...
  func addPark(park: Park) {
    parks.append(park)
  }
    
  func removeParkAtIndex(index: Int) {
     parks.removeAtIndex(index)
  }
  ...
  private func addTestData() {
    ...
    workout2.name = "Upper Body Workout"
    workout2.restInterval = 10
    workout2.exercises = [pushups, tricepsDips, pushupRotation]
    
    addWorkout(workout2)
    
    let workout3 = Workout()
    workout3.userCreated = false
    workout3.name = "Lower Body Workout"
    workout3.restInterval = 10
    workout3.exercises = [jumpingJacks, wallSit, stepups,
    ...
    workout4.name = "Abdominal Workout"
    workout4.restInterval = 10
    workout4.exercises = [crunches, plank, sidePlank]
    ...
    let park1 = Park()
    park1.userCreated = false
    park1.name = "Batalha Park"
    park1.location = "Batalha"
    park1.coordinates = "0,0"
    park1.category = "Maintenance Circuit"
    park1.photoFileName = "ortigosa"
    park1.rate = "4"
    park1.equipmentDetails = "Park equipped with various equipment such as fixed bars, parallel, monkeybars, bending bars, benchs, and more!"
    
    addPark(park1)
    
    
    let park2 = Park()
    park2.userCreated = false
    park2.name = "Lagoa Park"
    park2.location = "Ortigosa, Leiria"
    park2.category = "Maintenance Circuit"
    park2.coordinates = "0,0"
    park2.photoFileName = "ortigosa"
    park2.rate = "4"
    park2.equipmentDetails = "Park equipped with parallel, flat bars, benchs, monkey bars, wall bars, rings, among others."
